...
import {
    ...
    getUnpaidStoInvoices,
    getUnpaidOutletsWithDebt,
    processPaydown,
    FinancialAccount,
    UnpaidInvoice
...
export default function StoPaydownPage() {
    const { user } = useAuthUser()
    const [isLoading, setIsLoading] = useState(false)
    const [isSubmitting, setIsSubmitting] = useState(false)

    // Data State
    const [debtOutlets, setDebtOutlets] = useState<{ outlet_id: string, total_debt: number }[]>([])
    ...
    // Fetch Outlets and Accounts on Mount
    useEffect(() => {
        const initData = async () => {
            if (!user?.kode_outlet) return
            setIsLoading(true)
            const [outletsResult, accountsResult, allOutletsResult] = await Promise.all([
                getUnpaidOutletsWithDebt(user.kode_outlet),
                getFinancialAccounts(user.kode_outlet),
                masterOutletService.getAllOutlets()
            ])

            if (outletsResult.data) setDebtOutlets(outletsResult.data)
            if (accountsResult.data) setAccounts(accountsResult.data)
            if (allOutletsResult) setAllOutlets(allOutletsResult)
    ...
    // Fetch Invoices when Outlet changes
    useEffect(() => {
        if (!user?.kode_outlet || !selectedOutletId) {
            setInvoices([])
        ...
        const fetchInvoices = async () => {
            setIsLoading(true)
            const result = await getUnpaidStoInvoices(selectedOutletId, user.kode_outlet!)
            if (result.data) {
        ...
        fetchInvoices()
    }, [selectedOutletId, user?.kode_outlet])

    const getOutletName = (id: string) => {
        return allOutlets.find(o => o.kode_outlet === id)?.name_outlet || id
    }
    ...
    const handlePayment = async () => {
        if (!user?.kode_outlet || !user.id || !selectedOutletId || !paymentAccount) return
        ...
        // Call processPaydown with ref_outlet_id
        const result = await processPaydown({
            ...
            notes: notes,
            user_id: user.id,
            ref_outlet_id: selectedOutletId, // Important: Pass as ref_outlet_id
            supplier_id: null // Explicitly null
        })
        ...
        if (result.isSuccess) {
            ...
            // Refresh Data
            const invResult = await getUnpaidStoInvoices(selectedOutletId, user.kode_outlet)
            ...
            // Refresh debt list
            const outletsResult = await getUnpaidOutletsWithDebt(user.kode_outlet)
            if (outletsResult.data) setDebtOutlets(outletsResult.data)
    ...
    return (
        <DashboardLayout>
            <div className="max-w-6xl mx-auto space-y-6 pb-20">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900">STO Debt Settlement</h1>
                    <p className="text-muted-foreground mt-1">
                        Pay off inter-outlet invoices (Stock Transfers).
                    </p>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base text-gray-600 uppercase tracking-wider flex items-center gap-2">
                            <Store className="h-4 w-4" /> Step 1: Select Outlet
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            <SelectContent>
                                {debtOutlets.map(o => (
                                    <SelectItem key={o.outlet_id} value={o.outlet_id}>
                                        {getOutletName(o.outlet_id)} (Debt: {new Intl.NumberFormat('id-ID').format(o.total_debt)})
                                    </SelectItem>
                                ))}
                                {debtOutlets.length === 0 && (
                                    <SelectItem value="none" disabled>No outstanding inter-outlet debts found</SelectItem>
                                )}
                            </SelectContent>
                        </Select>
                    </CardContent>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-right w-[200px]">Discount</TableHead>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {accounts.length === 0 ? (
                                                <div className="p-2 text-sm text-red-500 bg-red-50 border border-red-200 rounded">
                                                    No Active Accounts. Create one in Finance &gt; Accounts.
                                                </div>
                                            ) : (
