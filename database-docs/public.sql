...
-- Function structure for process_bulk_payment
-- ----------------------------
DROP FUNCTION IF EXISTS "public"."process_bulk_payment"("p_outlet_id" text, "p_supplier_id" uuid, "p_account_id" uuid, "p_payment_date" date, "p_total_amount" numeric, "p_invoices" jsonb, "p_notes" text, "p_user_id" uuid);
CREATE OR REPLACE FUNCTION "public"."process_bulk_payment"("p_outlet_id" text, "p_supplier_id" uuid, "p_account_id" uuid, "p_payment_date" date, "p_total_amount" numeric, "p_invoices" jsonb, "p_notes" text=NULL::text, "p_user_id" uuid=NULL::uuid)
  RETURNS "pg_catalog"."json" AS $BODY$
DECLARE
    ...
    v_inv_record jsonb;
    v_invoice_id uuid;
    v_po_id uuid; -- To store linked PO ID
    v_alloc_amount numeric; -- Cash portion
    v_discount_amount numeric; -- Discount portion
    v_total_deduction numeric; -- Cash + Discount
    v_new_doc_number text;
BEGIN
    -- 1. Check Account Balance (Check against CASH amount only)
    SELECT balance INTO v_account_balance
    FROM public.master_financial_accounts
    WHERE id = p_account_id;

    IF v_account_balance < p_total_amount THEN
        RAISE EXCEPTION 'Insufficient funds in the selected account. Balance: %, Required: %', v_account_balance, p_total_amount;
    END IF;
    ...
    -- 4. Deduct Balance from Financial Account
    UPDATE public.master_financial_accounts
    ...
    LOOP
        ...
        -- C. Update Purchase Invoice Status & Get Linked PO ID
        -- We update Status to PAID if AP Ledger thinks it is verified paid
        WITH updated_invoice AS (
            ...
            RETURNING purchase_order_id, status
        )
        SELECT purchase_order_id INTO v_po_id 
        FROM updated_invoice
        WHERE status = 'PAID';

        -- D. Update Purchase Order Status (Using the fetched PO ID)
        -- If invoice became PAID, we mark PO as PAID.
        -- NOTE: This assumes 1 PO = 1 Invoice roughly, or we just mark it PAID if *this* invoice is paid.
        -- Ideally we check if *all* invoices for this PO are paid, but user asked for "after paid invoice -> po = PAID".
...
SELECT setval('"public"."barang_prices_id_seq"', 78, true);
...
SELECT setval('"public"."barang_units_id_seq"', 78, true);
...
SELECT setval('"public"."master_barang_id_seq"', 47, true);
...
-- ----------------------------
ALTER TABLE "public"."finance_ap_ledger" ADD CONSTRAINT "fk_ap_ledger_invoice" FOREIGN KEY ("invoice_id") REFERENCES "public"."purchase_invoices" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;
...
-- ----------------------------
ALTER TABLE "public"."finance_payment_allocations" ADD CONSTRAINT "alloc_invoice_fkey" FOREIGN KEY ("invoice_id") REFERENCES "public"."purchase_invoices" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION;
